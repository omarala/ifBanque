...
//		System.out.println(view.getPrincipal().getBtnVosCompte());
		...
		this.view.getPrincipal().getVirPanel().getBtnValider().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					boolean test = user.transferer(virPane.getDebiteur(), virPane.getCompteSelectionne(), doubleMontant);
					virPane.getDebiteur().majSolde();
					if(test == true) {
						JOptionPane.showMessageDialog(null, "virement effectué avec succès", "virement", JOptionPane.OK_CANCEL_OPTION);
					}else {
						JOptionPane.showMessageDialog(null, "virement non effectué, votre solde ne le permet pas", "virement", JOptionPane.OK_CANCEL_OPTION);
		...
		this.view.getPrincipal().getVirPanel().getCompteConnuDialog().addBtnValiderListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				CompteConnu newCompte = new CompteConnu(Integer.parseInt(dialog.getRib()), dialog.getNom(), dialog.getPrenom());
